# bot.py — AZAL Transit Bot + Google Gemini AI
# Источники: airport.az + AviationStack + Gemini API
...
log = logging.getLogger("azal-bot")
...
def parse_board(url: str) -> list:
    ...
    for a in soup.select('a[href*="?flight="]'):
        ...
        term_m   = re.search(r'Terminal\s*(\d)|T(\d)', raw, re.I)
        terminal = (term_m.group(1) or term_m.group(2)) if term_m else "-"
        ...
        flights.append({
            "airline": airline, "number": number,
            "code": f"J{number}" if airline == "J2" else f"{airline}{number}",
...
def build_card(f: dict) -> str:
    ...
    if cancelled:
        zaderjka, pribyl, opozdanie = "❌ рейс отменён", "❌ отменён", "—"
    elif arrived:
    ...
    return (
        ...
        f"🕐 <b>Время:</b> {f['sched']} (по расписанию)\n"
        f"🏢 <b>Терминал:</b> {term}"
    )
...
def aviationstack_card(number: str):
    ...
    try:
        ...
        return {
            ...
            "city": arr.get("airport", "—"),
            "sched": sched, "actual": actual,
            "status": d.get("flight_status", ""),
            "terminal": arr.get("terminal", "-") or "-",
        }
    except Exception as e:
        log.warning("AviationStack: %s", e)
        return None


def find_flight(number: str):
    all_f = parse_board(ARRIVALS_URL) + parse_board(DEPARTURES_URL)
    for f in all_f:
        if f["number"] == number and f["airline"] == "J2":
            return f
    return aviationstack_card(number)


# ── Google Gemini ─────────────────────────────────────────────

def gemini_ask(user_question: str, flights_context: str) -> str:
    """Отправляет вопрос пользователя + данные рейсов в Gemini и возвращает ответ."""
    if not GEMINI_KEY:
        return "Gemini API ключ не задан (GEMINI_KEY)."

    system_prompt = (
        "Ты — помощник диспетчера транзитных рейсов в аэропорту Гейдар Алиев (Баку, GYD). "
        "Отвечай коротко, по-русски. Используй только данные из списка рейсов ниже. "
        "Если рейс не найден — так и скажи.\n\n"
        f"Текущие рейсы AZAL (J2):\n{flights_context}"
    )
        ...
        resp = requests.post(
            ...
            json={
                "contents": [
                    ...
                    {"role": "model", "parts": [{"text": "Понял, готов отвечать по рейсам."}]},
...
def build_flights_context() -> str:
    """Собирает краткий текст всех рейсов для контекста Gemini."""
    arr = [f for f in parse_board(ARRIVALS_URL)   if f["airline"] == "J2"]
    dep = [f for f in parse_board(DEPARTURES_URL) if f["airline"] == "J2"]
    lines = []
    for f in arr:
        d = delay_min(f["sched"], f["actual"])
        delay_str = f"+{d}мин" if d and d > 0 else "вовремя"
        lines.append(
            f"[ПРИЛЁТ] {f['code']} {f['city']} план={f['sched']} "
            f"факт={f['actual'] or '—'} статус={f['status']} "
            f"задержка={delay_str} терминал={f['terminal']}"
        )
    for f in dep:
        d = delay_min(f["sched"], f["actual"])
        delay_str = f"+{d}мин" if d and d > 0 else "вовремя"
        lines.append(
            f"[ВЫЛЕТ] {f['code']} {f['city']} план={f['sched']} "
            f"факт={f['actual'] or '—'} статус={f['status']} "
            f"задержка={delay_str} терминал={f['terminal']}"
        )
...
# Состояние: ожидаем ли вопрос для AI
ai_wait = set()
...
async def cmd_start(message: Message):
    await message.answer(
        ...
        "🛬 Прилёт / 🛫 Вылет — список рейсов AZAL\n"
        "⚠️ Задержки — опоздания и отмены\n"
        "🤖 Спросить AI — вопрос на живом языке\n\n"
        "Или отправь номер рейса: <code>278</code> / <code>J278</code>",
...
async def ask_ai_prompt(message: Message):
    ...
    await message.answer(
        "🤖 Задай вопрос о рейсах на живом языке.\n"
        "Например: <i>Когда прилетает рейс из Стамбула?</i>\n"
        "или <i>Есть задержки сегодня?</i>"
    )
...
async def by_number(message: Message):
    ...
    if f:
        ...
        await msg.edit_text(
            f"❓ Рейс J{number} не найден в ближайшем окне.\n"
            f"Flightradar24: https://www.flightradar24.com/J2{number}"
        )
...
async def fallback(message: Message):
    ...
    # Если пользователь нажал "Спросить AI" — отправляем вопрос в Gemini
    if uid in ai_wait:
        ai_wait.discard(uid)
        msg = await message.answer("🤖 Думаю…")
        context  = build_flights_context()
        answer   = gemini_ask(message.text, context)
        await msg.edit_text(f"🤖 <b>AI:</b>\n{answer}")
        return

    await message.answer(
        "Отправь номер рейса: <code>278</code> или <code>J278</code>\n"
        "Или нажми 🤖 Спросить AI",
        reply_markup=main_menu,
    )
