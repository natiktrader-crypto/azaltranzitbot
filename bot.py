...
def gemini_ask(question: str, ctx: str) -> str:
    if not GEMINI_KEY:
        return "GEMINI_KEY не задан в Railway Variables."
    prompt = (
    ...
    try:
        r = requests.post(
            ...
            json={"contents": [
                ...
                {"role": "model", "parts": [{"text": "Понял, отвечаю по рейсам."}]},
...
async def fallback(message: Message):
    ...
    if uid in ai_wait:
        ...
        ctx    = flights_context()
        answer = gemini_ask(message.text, ctx)
