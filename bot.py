# bot.py — AZAL Transit Bot
# Формат карточки: Команда / Направление / Задержка / Прибыл / Опоздание / Время / Терминал
# Источники: airport.az + AviationStack API
...
def parse_board(url: str) -> list:
    ...
    for a in soup.select('a[href*="?flight="]'):
        ...
        cm = re.search(r'\b([A-Z][A-Z0-9])\s?(\d{2,4})\b', raw)
        if not cm:
            continue
...
def aviationstack_card(number: str):
    ...
    try:
        ...
        return {
            ...
            "status": status, "terminal": arr.get("terminal","-") or "-",
        }
    except Exception as e:
...
main_menu = ReplyKeyboardMarkup(
    keyboard=[
        ...
        [KeyboardButton(text="⚠️ Задержки")],
    ],
...
@dp.message(F.text.regexp(r'^[Jj]?2?\s?(\d{2,4})$'))
async def by_number(message: Message):
    m = re.search(r'(\d{2,4})$', message.text.strip())
    if not m:
        return
    number = m.group(1)
